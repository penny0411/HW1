...
# ─── Custom CSS (Fixed white bar issue) ───
st.markdown("""
<style>
    @import url('https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800&display=swap');
    
    /* 徹底移除 Streamlit 預設頂部區塊 */
    ...
    .header-title { color: #ffffff; font-size: 1.8rem; font-weight: 700; display: flex; align-items: center; gap: 10px; }
...
def live_monitor():
    ...
    fig_temp = go.Figure()
    fig_temp.add_trace(go.Scatter(x=df['timestamp'], y=df['temperature'], line=dict(color='#f59e0b', width=3), fill='tozeroy', fillcolor='rgba(245, 158, 11, 0.1)'))
    fig_temp.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', height=250, margin=dict(l=10,r=10,t=10,b=10), showlegend=False, xaxis=dict(showgrid=False), yaxis=dict(showgrid=True, gridcolor='rgba(255,255,255,0.05)'))
    st.plotly_chart(fig_temp, use_container_width=True)

    fig_humid = go.Figure()
    fig_humid.add_trace(go.Scatter(x=df['timestamp'], y=df['humidity'], line=dict(color='#3b82f6', width=3), fill='tozeroy', fillcolor='rgba(59, 130, 246, 0.1)'))
    fig_humid.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', height=250, margin=dict(l=10,r=10,t=10,b=10), showlegend=False, xaxis=dict(showgrid=False), yaxis=dict(showgrid=True, gridcolor='rgba(255,255,255,0.05)'))
